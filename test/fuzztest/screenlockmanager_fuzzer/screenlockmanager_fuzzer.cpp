...
bool FuzzRegisterStrongAuthListener(const uint8_t *rawData, size_t size)
{
    ...
    sptr<StrongAuthListener> StrongAuthListenerTest1 = new (std::nothrow) StrongAuthListenerTest(userId);
    int32_t ret = ScreenLockManager::GetInstance()->RegisterStrongAuthListener(StrongAuthListenerTest1);
    return ret == E_SCREENLOCK_OK;
}

bool FuzzUnRegisterStrongAuthListener(const uint8_t *rawData, size_t size)
{
    if (size < LENGTH) {
        return true;
    }
    int32_t userId = rawData[0];
    sptr<StrongAuthListener> StrongAuthListenerTest1 = new (std::nothrow) StrongAuthListenerTest(userId);
    int32_t ret = ScreenLockManager::GetInstance()->UnRegisterStrongAuthListener(StrongAuthListenerTest1);
    return ret == E_SCREENLOCK_OK;
}
...
bool FuzzRegisterDeviceLockedListener(const uint8_t *rawData, size_t size)
{
    ...
    int32_t userId = rawData[0];
    sptr<DeviceLockedListener> DeviceLockedListenerTest1 = new (std::nothrow) DeviceLockedListenerTest(userId);
    int32_t ret = ScreenLockManager::GetInstance()->RegisterDeviceLockedListener(DeviceLockedListenerTest1);
    return ret == E_SCREENLOCK_OK;
}

bool FuzzUnRegisterDeviceLockedListener(const uint8_t *rawData, size_t size)
{
    if (size < LENGTH) {
        return true;
    }
    int32_t userId = rawData[0];
    sptr<DeviceLockedListener> DeviceLockedListenerTest1 = new (std::nothrow) DeviceLockedListenerTest(userId);
    int32_t ret = ScreenLockManager::GetInstance()->UnRegisterDeviceLockedListener(DeviceLockedListenerTest1);
    return ret == E_SCREENLOCK_OK;
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::FuzzUnRegisterStrongAuthListener(data, size);
    OHOS::FuzzRegisterDeviceLockedListener(data, size);
    OHOS::FuzzUnRegisterDeviceLockedListener(data, size);
